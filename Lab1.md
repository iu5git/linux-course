...
1. Качаем образ для установки дистрибутива Debian.
Я обычно делаю это с [зеркала МИФИ][debian-mirror]. Впрочем, вы можете выбрать любой способ загрузки, подходящий вам, например через [BitTorrent][debian-torrent]. Все варианты можно узнать [тут][debian-all-mirrors].

Во время подготовки курса использовался [образ][debian-iso] `debian-11.2.0-amd64-DVD-1.iso`, как наиболее универсальный вариант. Но с серверов он выкачивался целую вечность. Лучше использовать BitTorrent.
...
4. Запускаем виртуальную машину и устанавливаем систему. Подробно рассматривать процесс установки не будем, остановимся только на самых важных деталях. Для интересующихся руководство по установке Debian для x86-64 архитектуры вы можете найти [тут][debian-install].
   ...
   ![Загрузились с образа](./s5-installer.png)
   ...
   + Далее установщик попросит вас ввести `hostname` - [сетевое имя системы][hostname]. На ваше усмотрение. Я выберу `enterprise`.
   + Теперь `domain`, для ВМ я обычно использую `vm`.
   + Пароль [root][root]. Пользователя с идентификатором (UID - user ID) 0 и имеющим неограниченные привилегии. Аналог администратора. Далее будем называть его root. Настоятельно не советую ставить свой настоящий сложный пароль на ВМ. После ввода его попросят еще раз для подтверждения, что вы в своем уме.
   + Имя вашего аккаунта и имя пользователя. Выбирайте что-то простое и емкое. Я использую `mak` для всего.
   ...
   + Подтверждение, что вы действительно хотите записать изменения на диск. Выбираем `Yes`. У меня получились вот такие разделы: [разделы](./s6-lvm)
   ...
   + Страна, для определения списка ближайших зеркал. Выбираем `Russia`, а далее понравившийся. Я все же буду использовать `deb.debian.org`, привычнее.
   ...
   + Выбираем этот диск (/dev/sda), он у нас один. ![Примерно так](./s7-grub.png)
   ...
   + Пробуем войти своим пользователем. Если все введено верно, то вы увидите приглашение ввода команд (command prompt), например: `mak@enterprise:~$ `. ВМ "на логине". Критический успех!![На логине](./s8-onlogin.png)
...
5. Делаем мгновенный снимок виртуальной машины, если ваша система виртуализации это поддерживает. Тогда в любой момент мы сможем вернуться на данное состояние системы, если что-то сломаем. В VB переходим во вкладку "Снимки" (Snapshots) и нажимаем "сделать". Названия не принципиальны.
   ...
   ![Снапшоты 1](./s9-snapshots.png)

   ![Снапшоты 2](./s9-snapshots-2.png)
...
7. Перезагружаемся (reset) с помощью VB. ВМ мгновенно перезапустится (будто мы обесточили и снова запустили сервер). Если все в порядке - вы снова окажетесь "на логине". ![Перезагрузка](./s12-reset.png)
...
3. Отредактируйте список репозиториев `/etc/apt/sources.list` с помощью любого текстового редактора.
   ...
   ![Пример файла sources.list](./s13-apt.png)
...
6. Напишите "исполняемый" bash-скрипт (лучше sh), который создает в текущей директории файлы `test/file${N}.txt` с содержимым `Hello${N}\n`, где `${N}` заменить на число, пробегающее [X; Y], если они еще не созданы. X и Y задаются как 1 и 2 аргументы соответсвенно.
   ...
   >
   > Все дело в безопасности. Это очень плохая идея позволять подменять стандартные утилиты файлами из текущей директории, есть риск исполнить не то, что подразумевал пользователь. Поэтому текущая директория не добавляется к переменной PATH, списку директорий в которых производится поиск исполняемых файлов.
...
1. Настроить "проброс портов" для виртуальной машины **127.0.0.1:2222 -> 10.0.2.15:22**
   ...
   ![Port forwarding](./s14-nat.png)

   ![SSH-rule](./s15-ssh-rule.png)

2. На **ВМ** - установить и запустить sshd (проверить запущен ли: `systemctl status ssh`)
...
3. На **основной** системе настроить ssh-подключение по ключу
   ...
   + Сгенерировать ssh-ключи (в утилите [PuTTYgen][puttygen] или `ssh-keygen`)
     ...
     ![PuTTYgen](./s18-win-keygen-1.png)

     ![PuTTYgen](./s18-win-keygen-2.png)

     Unix/Linux:

     ![ssh-keygen](./s19-ssh-keygen.png)
...
5. Показать soft-лимиты (`ulimit`) и hard-лимиты, пояснить как поняли, в чем разница, как изменить?
   ...
   ![sshd-limits](./s23-sshd-limits.png)

   Занятное наблюдение работы лимитов:

   ![limit-check](./s24-limit-check.png)
...
8. **Дополнительное задание**: настроить `/root/.bashrc` файл, применяющийся при создании новой оболочки для root пользователя.
   ...
   ![.bashrc](./s26-bashrc.png)

   ![Colors](./s27-colors.png)
